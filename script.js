document.addEventListener('DOMContentLoaded', () => {
    ...
    function addInstance() {
        ...
        instance.innerHTML = `
            ...
            <div class="row">
                ...
                <div class="cell"><input type="text" placeholder="Charge"></div> <!-- Renamed "Charges" to "Charge" -->
            </div>
            <div class="row">
                <div class="cell"><input type="number" class="numeric-spinner" min="0" max="99"></div>
                <div class="cell"><input type="number" class="numeric-spinner" min="0" max="99"></div>
    ...
    function snapAllInstances() {
        ...
        instances.forEach((instance, index) => {
            const row = Math.floor(index / 2); // Adjusted for doubled width
            const col = index % 2; // Adjusted for doubled width
            const snappedPos = {
    ...
    function generateValues(instance) {
        ...
        numericInputs.forEach(input => {
            ...
            const usedRolls = rolls.filter(roll => roll !== dropped).sort((a, b) => b - a);
            const sum = usedRolls.reduce((a, b) => a + b, 0);
            input.value = sum;
            input.title = `${usedRolls.join(', ')} (${dropped})`;
        });
    }
